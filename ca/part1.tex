...
\begin{itemize}
\item Paquets per a treballs, presentacions, fulls de càlcul, \ldots
\end{itemize}
\end{itemize}
\end{frame}
...
La pluja cau \emph{principalment} a la plana.
\end{minted}
\vskip 2ex
\tikz\node[single arrow,fill=gray,font=\ttfamily\bfseries,%
  rotate=270,xshift=-1em]{latex};
\vskip 2ex
\fbox{La pluja cau \emph{principalment} a la plana.}
\end{center}
\end{frame}
...
\item `Compila' el teu \LaTeX{}  automàticament per a mostrar-te els resultats. 
\vskip 2em
\begin{center}
...
\item Per exemple, si escriviu malament \cmdbs{emph} com a \cmdbs{meph}, \LaTeX{} s'aturarà amb un error 
``undefined control sequence'', perquè ``meph'' no és una de les ordres que coneix.
\end{itemize}
\begin{block}{Consells sobre Errors}
\begin{enumerate}
\item No t'espantis! És comú trobar errors.
\item Arregleu-los tan aviat com apareguin, si el que acabeu d'escriure
ha provocat un error, comenceu per allà. 
\item Si hi ha diversos errors, comenceu pel primer:
...
positius, i sigui $c = a - b + 1$.
\end{exampletwouptiny}
\item Utilitza sempre els símbols de dòlar amb parelles -- u per començar les matemàtiques, i un per acabar-les.
